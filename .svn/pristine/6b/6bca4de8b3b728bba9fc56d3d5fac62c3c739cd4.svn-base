<?php

use CodeIgniter\Router\RouteCollection;

/** @var RouteCollection $routes */
$routes->get('/', 'Home::index');
$routes->get('test-db', 'Test::index');

// Login Routes
$routes->post(
    'login',
    'LoginController::login'
);
$routes->post(
    'login-crypto-config',
    'LoginController::cryptoConfig'
);
$routes->post('verify-otp', 'LoginController::verifyOtp');
$routes->get(
    'logout',
    'LoginController::logout'
);
$routes->post(
    'refresh-captcha',
    'LoginController::refreshCaptcha'
);
$routes->get(
    'contact',
    'LoginController::contact'
);
$routes->get(
    'change-password',
    'ChangePassword::index'
);
$routes->post(
    'change-password/update',
    'ChangePassword::update'
);

// Signup Auth Routes
$routes->get('signup', 'Auth::signup');
$routes->post('auth/register', 'Auth::register');
$routes->get(
    'auth/authorization',
    'Auth::authorization'
);
$routes->post(
    'auth/upload-authorization',
    'Auth::uploadAuthorization'
);
$routes->get(
    'auth/application-submitted',
    'Auth::applicationSubmitted'
);
$routes->post('refresh-captcha', 'Auth::refreshCaptcha');


$routes->get('dashboard', 'Dashboard::index');

// Request routes
$routes->get('new-request', 'RequestController::newRequest');
$routes->get('getVendors', 'RequestController::getVendors');
$routes->post('getJammerModelsByVendor', 'RequestController::getJammerModelsByVendor');
$routes->post('submit-request', 'RequestController::submitRequest');
$routes->get('view-request/(:num)', 'RequestController::viewRequest/$1');
$routes->get('get-request/(:num)', 'RequestController::getRequest/$1');


// View & upload-signed-pdf routes
$routes->get('request-view/(:num)', 'RequestViewController::index/$1');
$routes->get('request-view', 'RequestViewController::index');
$routes->post('forward', 'RequestViewController::forward');
$routes->post('reject', 'RequestViewController::reject');
$routes->get('application-pdf/(:num)', 'RequestViewController::getApplicationPdf/$1');
$routes->get('preview-application-pdf/(:num)', 'RequestViewController::previewApplicationPdf/$1');
$routes->get('permission-preview-application-pdf/(:num)', 'RequestViewController::permission_previewApplicationPdf/$1');
$routes->get('download-application-pdf/(:num)', 'RequestViewController::downloadApplicationPdf/$1');
$routes->post('upload-signed-pdf', 'RequestViewController::uploadSignedPdf');
$routes->post('upload-permission-letter', 'RequestViewController::uploadPermissionLetter');


// Application PDF routes
$routes->get('preview-application-pdf/(:num)', 'RequestViewController::previewApplicationPdf');
$routes->get('download-application-pdf/(:num)', 'RequestViewController::downloadApplicationPdf');
$routes->post('upload-signed-pdf', 'RequestViewController::uploadSignedPdf');
$routes->get('application-pdf/(:num)', 'RequestViewController::getApplicationPdf');



// location Routes
$routes->get(
    'location/get-states',
    'LocationController::getStates'
);
$routes->post(
    'location/get-cities',
    'LocationController::getCities'
);

// Document Routes
$routes->get(
    'view-document/(:num)',
    'RequestViewController::viewDocument/$1'
);

$routes->get(
    'download-document/(:num)',
    'RequestViewController::downloadDocument/$1'
);


// Organizations Routes
$routes->get('organizations', 'Dashboard::organizations');
$routes->get('get-organizations', 'Dashboard::getOrganizations');
$routes->post('save-organization', 'Dashboard::saveOrganization');
$routes->get('get-organization/(:num)', 'Dashboard::getOrganization/$1');
$routes->post('delete-organization/(:num)', 'Dashboard::deleteOrganization/$1');

// Organization Types Routes
$routes->get('organization-types', 'Dashboard::orgTypes');
$routes->get('get-org-types', 'Dashboard::getOrgTypes');
$routes->post('save-org-type', 'Dashboard::saveOrgType');
$routes->get('get-org-type/(:num)', 'Dashboard::getOrgType/$1');
$routes->post('delete-org-type/(:num)', 'Dashboard::deleteOrgType/$1');

// Vendor Routes
$routes->get('vendors', 'Dashboard::vendors');
$routes->get('get-vendors', 'Dashboard::getVendors');
$routes->get('get-vendor/(:num)', 'Dashboard::getVendor/$1');
$routes->post('save-vendor', 'Dashboard::saveVendor');
$routes->post('delete-vendor/(:num)', 'Dashboard::deleteVendor/$1');

// Model Routes
$routes->get('models', 'Dashboard::models');
$routes->get('get-models', 'Dashboard::getModels');
$routes->get('get-model/(:num)', 'Dashboard::getModel/$1');
$routes->post('save-model', 'Dashboard::saveModel');
$routes->post('delete-model/(:num)', 'Dashboard::deleteModel/$1');

// User Routes
$routes->get('users', 'Dashboard::users');
$routes->get('get-users', 'Dashboard::getUsers');
$routes->post('save-user', 'Dashboard::saveUser');
$routes->get('get-user/(:num)', 'Dashboard::getUser/$1');
$routes->post('delete-user/(:num)', 'Dashboard::deleteUser/$1');
$routes->post('reset-user-password/(:num)', 'Dashboard::resetUserPassword/$1');
$routes->post('toggle-lock-user/(:num)', 'Dashboard::toggleLockUser/$1');

// Registration Routes
$routes->get('registrations', 'Dashboard::registrations');
$routes->post('approve-registration', 'Dashboard::approveRegistration');

// Setting Routes
$routes->get('settings', 'SettingController::index');
$routes->get('get-settings', 'SettingController::getSettings');
$routes->get('get-setting/(:num)', 'SettingController::getSetting/$1');
$routes->post('save-setting', 'SettingController::saveSetting');
$routes->post('delete-setting/(:num)', 'SettingController::deleteSetting/$1');

// Additional Dashboard Routes
$routes->get('requests', 'Dashboard::requests');
$routes->get('analytics', 'Dashboard::analytics');
$routes->get('audit-trail', 'AuditController::auditTrail');
$routes->get('get-audit-logs', 'AuditController::getAuditLogs');
$routes->get('audit-log', 'AuditController::auditLog');
$routes->get('get-audit-actions', 'AuditController::getAuditActions');

$routes->get('uploads/authorization/(:segment)', 'PdfController::viewPdf/$1');