...
<section class="stats-grid">
    <?php 
    $hasHigherRole = $hasRole([2, 3, 4, 5, 6, 7, 8, 9]); 
    ?>

    <?php if ($hasHigherRole): ?>
                ...
                <table class="w-full text-sm gov-table rounded-lg overflow-hidden" id="dashboardRequestsTable">
                    <thead class="bg-[#1e4d7b] text-white">
                        <tr>
                            ...
                            <th class="px-5 py-3.5 text-left font-semibold uppercase tracking-wider text-xs">Center List Ready</th>
                        ...
                        <?php if (!empty($requests) && is_array($requests)): ?>
                            <?php foreach ($requests as $request): ?>
                                <tr class="hover:bg-slate-50 transition-colors duration-150">
                                    ...
                                    <td class="px-5 py-4 text-left">
                                        <?php if (!empty($request['centre_list_ready']) && $request['centre_list_ready'] == 1): ?>
                                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-800 border border-green-300">
                                                <i class="fas fa-check-circle text-green-600"></i> Yes
                                            </span>
                                        <?php else: ?>
                                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-800 border border-red-300">
                                                <i class="fas fa-times-circle text-red-600"></i> No
                                            </span>
                                        <?php endif; ?>
                                    </td>
