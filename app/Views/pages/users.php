                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors duration-150">
                                ...
                                <td class="px-5 py-4 text-right pr-6">
                                    <div class="flex justify-start gap-2">
                                        ...
                                        <button class="w-8 h-8 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 border border-red-100 transition delete-user-btn flex items-center justify-center" data-id="<?= $user['id'] ?>" data-name="<?= esc($user['name']) ?>" data-email="<?= esc($user['email']) ?>" title="Delete">
...
$(document).ready(function() {
        ...
        if(!users || users.length === 0) {
            ...
            users.forEach(function(user, index) {
                ...
                let row = '<tr class="hover:bg-slate-50/80 transition-colors duration-150">' +
                    ...
                    '<td class="px-5 py-4 font-bold text-slate-800">' + userName + '</td>' +
                    '<td class="px-5 py-4 text-slate-600 font-medium">' + userEmail + '</td>' +
    ...
    function attachUserEventListeners() {
        ...
        $(document).off('click', '.delete-user-btn').on('click', '.delete-user-btn', function(e) {
            ...
            let userEmail = $(this).data('email') ? ` (${$(this).data('email')})` : '';

            Swal.fire({
                title: 'Deactivate User?',
                text: `Are you sure you want to deactivate "${userName}"${userEmail}?`,
                icon: 'warning',
        ...
        $(document).off('click', '.reset-user-btn').on('click', '.reset-user-btn', function(e) {
            e.preventDefault();
            let id = $(this).data('id');
            let userName = $(this).data('name') || 'this user';
    ...
    $(document).off('click', '.lock-user-btn').on('click', '.lock-user-btn', function(e) {
        ...
        let targetStatus = currentStatus === 1 ? 0 : 1; 
        let actionText = targetStatus === 1 ? 'lock' : 'unlock';
        let confirmBtnText = targetStatus === 1 ? 'Yes, Lock User' : 'Yes, Unlock User';
        ...
        Swal.fire({
            ...
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: targetStatus === 1 ? '#dc2626' : '#e58500',
            cancelButtonColor: '#64748b',
            ...
            if (result.isConfirmed) {
                ...
                let dataParam = {};
                dataParam[csrfInput.attr('name')] = csrfInput.val();
                dataParam['status'] = targetStatus;

                $.ajax({
                    url: "<?= base_url('toggle-lock-user/') ?>" + id,
                    type: "POST",
                    data: dataParam,
                    dataType: "json",
                    success: function(res) {
