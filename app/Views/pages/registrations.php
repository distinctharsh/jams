    ...
    <div class="gov-card p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-xl font-bold text-[#1e4d7b]">Registration Approval Management</h2>
                ...
                <tbody class="divide-y divide-slate-100 bg-white text-slate-700">
                    <?php if(!empty($registrations)): ?>
                        <?php foreach($registrations as $reg): 
                            $status = intval($reg['current_status'] ?? 1);
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors duration-150">
                                ...
                                <td class="px-5 py-4 text-left">
                                    <?php if(!empty($reg['authorization_letter'])): ?>
                                        <a href="<?= base_url('uploads/authorization/'.$reg['authorization_letter']) ?>" target="_blank" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 text-[#1e4d7b] transition shadow-sm" title="View Authorization Letter">
                                ...
                                <td class="px-5 py-4 text-left pr-6">
                                    <?php if(in_array($status, [1, 2, 3])): ?>
                                        <div class="flex items-center gap-2">
                                            <!-- Approve Button: Disabled for Status 1 and 2 -->
                                            <?php if(in_array($status, [1, 2])): ?>
                                                <button disabled class="px-3 py-1.5 rounded-lg bg-slate-200 text-slate-400 cursor-not-allowed border border-slate-300 text-xs font-semibold flex items-center gap-1 shadow-sm" title="Cannot approve at this stage">
                                    ...
                                    <?php elseif($status == 4): ?>  
                                            <button class="px-3 py-1.5 rounded-lg bg-amber-50 text-amber-700 hover:bg-amber-100 border border-amber-200 transition text-xs font-semibold reset-pwd-btn flex items-center gap-1.5 shadow-sm" data-id="<?= $reg['id'] ?>" data-name="<?= esc($reg['name']) ?>" data-email="<?= esc($reg['email']) ?>">
...
$(document).ready(function() {
    ...
    function initDataTable() {
        if ($.fn && $.fn.DataTable) {
            ...
            // Default filter to Pending (1, 2, 3)
    ...
    $(document).on('click', '.reset-pwd-btn', function(e) {
        ...
        let userName = $(this).data('name') || 'this applicant';

        Swal.fire({
            title: 'Reset Password?',
            text: `Are you sure you want to reset password for "${userName}" (${email})?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#e58500',
                ...
                $.ajax({
                    ...
                    error: function() {
                        if(typeof showToast === 'function') {
                            ...
                            Swal.fire('Error', 'Error occurred while resetting password.', 'error');
                        }
                    }
                });
            }
    ...
    $('#actionForm').submit(function(e) {
        ...
        let actionText = (actionType == 4) ? 'Approve' : 'Reject';
        let confirmBtnColor = (actionType == 4) ? '#16a34a' : '#dc2626';

        Swal.fire({
            title: `${actionText} Registration?`,
            text: `Are you sure you want to confirm ${actionText.toLowerCase()} action for this registration?`,
            icon: actionType == 4 ? 'question' : 'warning',
            showCancelButton: true,
