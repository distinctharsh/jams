...
class UserModel extends Model
{
    ...
    public function getAllUsers()
    {
        return $this->db->table($this->table)
            ->select('user.*, mas_organization.org_name, mas_organization_type.name as org_type_name, GROUP_CONCAT(DISTINCT mas_role.name ORDER BY mas_role.name SEPARATOR ", ") AS role_name, GROUP_CONCAT(DISTINCT mas_role.id ORDER BY mas_role.id SEPARATOR ", ") AS role_ids')
